...
document.getElementById('todo-form').addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch('http://localhost:3000/api/todo/create-todo', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
...
async function loadTodos() {
    const token = localStorage.getItem('token');
    try {
        const response = await fetch('http://localhost:3000/api/user/get-todos', {
            headers: {
                'Authorization': `Bearer ${token}`,
            },
        });
...
async function completeTodo(id, completed) {
    const token = localStorage.getItem('token');
    try {
        await fetch(`http://localhost:3000/api/todo/update-todo`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`,
